 ...
 * 
 * @param bufs Number of buffer frames in the buffer pool.
 */
...
/**
 * @brief Destructor for the buffer manager.
 * 
 * Writes all dirty pages back to disk and frees allocated memory.
 */
BufMgr::~BufMgr() {
	// 遍历整个缓冲区
...
/**
 * @brief Advances the clock hand to the next frame.
 * 
 * Ensures circular traversal within the buffer pool.
 */
...
/**
 * @brief Allocates a free buffer frame using the clock replacement policy.
 * 
 * @param frame Reference to a variable where the allocated frame number is returned.
 * @throws BufferExceededException If all frames are pinned.
 */
...
/**
 * @brief Reads a page into the buffer pool.
 * 
 * @param file Pointer to the file containing the page.
 * @param pageNo Page number to read.
 * @param page Pointer to the buffer pool frame containing the page (output).
 * @throws HashNotFoundException If the page is not found in the buffer pool.
 */
 ...
 * @param dirty If true, marks the page as dirty.
 * @throws PageNotPinnedException If the page is not pinned.
 */
...
/**
 * @brief Flushes all pages belonging to a specific file from the buffer pool.
 * 
 * Writes all dirty pages back to disk, clears their state, and removes them from the hash table.
 * 
 * @param file Pointer to the file whose pages need to be flushed.
 * @throws PagePinnedException If any of the pages in the file are pinned.
 * @throws BadBufferException If any of the pages are invalid.
 */
...
/**
 * @brief Allocates a new page in the specified file and maps it to the buffer pool.
 * 
 * Allocates a page in the file, assigns it a free frame in the buffer pool, and updates metadata.
 * 
 * @param file Pointer to the file in which a new page is to be allocated.
 * @param pageNo Reference to return the allocated page number.
 * @param page Pointer to the buffer pool frame containing the new page (output).
 */
...
/**
 * @brief Disposes of a specific page in the file.
 * 
 * Removes the page from the buffer pool (if present) and deletes it from the file.
 * 
 * @param file Pointer to the file containing the page.
 * @param PageNo Page number to be deleted.
 */
...
/**
 * @brief Prints the current state of the buffer pool.
 * 
 * Outputs the state of each frame, including its validity and metadata.
 */
